...
if __name__ == '__main__':
    ...
    # Параметры слоев
    layers_cont = [LayerContinuous(xmin=0, eps=4.0, sigma=0.0)]
    ...
    # Источник
    magnitude = 1.0
    signal = GaussianMod(1.0, 30, 15, Nl, 1.0)
    ...
    plt.plot(freq[:100], abs(sp[:100])/max(abs(sp[:100])))
    plt.xlabel('f, ГГц')
    display.stop()
    plt.show()
